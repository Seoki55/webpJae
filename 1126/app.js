...
(function () {
  ...
  // State
  // Spotify token (로컬에 저장된 토큰 복원)
  let accessToken = localStorage.getItem('spotify.accessToken') || null;
  ...
  // 안전한 Spotify 버튼 상태 업데이트 (ReferenceError 방지)
  function updateSpotifyButtons() {
    try {
      // 페이지에 Spotify 관련 버튼이 있으면 상태를 반영합니다.
      const spotifyLoginBtn = document.getElementById('spotify-login');
      const spotifyLogoutBtn = document.getElementById('spotify-logout');
      if (spotifyLoginBtn) spotifyLoginBtn.disabled = !!accessToken;
      if (spotifyLogoutBtn) spotifyLogoutBtn.disabled = !accessToken;
    } catch (e) {
      console.warn('updateSpotifyButtons:', e);
    }
  }
  // 초기 호출(안전)
